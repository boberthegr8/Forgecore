...
on public.portal_access_grants for all to authenticated
using (
  exists (
    select 1 from public.organization_memberships m
    where m.organization_id = portal_access_grants.organization_id
...
begin
  ...
  if p_project_id is not null then
    select p.organization_id into v_project_org from public.projects p where p.id = p_project_id;
    ...
    select c.organization_id into v_customer_org from public.customers c where c.id = p_customer_id;
  ...
  if p_project_id is not null then
    ...
    do update set status='active', access_level=excluded.access_level,
      can_view_quotes=excluded.can_view_quotes, can_view_documents=excluded.can_view_documents,
      can_view_deliveries=excluded.can_view_deliveries, can_approve=excluded.can_approve,
  ...
  insert into public.events (
    organization_id, entity_type, entity_id, action, payload, source, actor_user_id
  ) values (
    p_organization_id, 'portal_access_grant', v_grant_id, 'portal_access_granted',
...
with active_grants as (
  select g.* from public.portal_access_grants g
  where g.user_id = auth.uid()
    and g.status = 'active'
    and (g.expires_at is null or g.expires_at > now())
), project_access as (
  select p.id,p.organization_id,p.customer_id,p.project_number,p.name,p.status,p.description,p.address,p.updated_at,
  ...
  join public.projects p on p.organization_id=g.organization_id
   and ((g.project_id is not null and p.id=g.project_id) or (g.customer_id is not null and p.customer_id=g.customer_id))
  group by p.id
), project_rows as (
  select jsonb_build_object(
    'id',pa.id,'organization_id',pa.organization_id,'organization_name',o.name,
    'customer_id',pa.customer_id,'customer_name',c.display_name,
    'project_number',pa.project_number,'name',pa.name,'status',pa.status,
    'description',pa.description,'address',pa.address,'updated_at',pa.updated_at,
    'permissions',jsonb_build_object('quotes',pa.can_view_quotes,'documents',pa.can_view_documents,'deliveries',pa.can_view_deliveries,'approve',pa.can_approve),
    'quotes',case when pa.can_view_quotes then coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',q.id,'quote_number',q.quote_number,'status',q.status,'title',q.title,
        'current_revision',q.current_revision,'currency',q.currency,'subtotal',q.subtotal,
        'tax',q.tax,'total',q.total,'quote_date',q.quote_date,'expiry_date',q.expiry_date,'updated_at',q.updated_at
      ) order by q.updated_at desc)
      from public.quotes q where q.organization_id=pa.organization_id and q.project_id=pa.id
    ),'[]'::jsonb) else '[]'::jsonb end,
    'deliveries',case when pa.can_view_deliveries then coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',d.id,'delivery_number',d.delivery_number,'status',d.status,'scheduled_start',d.scheduled_start,
        'scheduled_end',d.scheduled_end,'direction',d.direction,'address',d.address,'truck',d.truck,
        'driver',d.driver,'load_type',d.load_type,'notes',d.notes
      ) order by d.scheduled_start nulls last)
      from public.deliveries d where d.organization_id=pa.organization_id and d.project_id=pa.id
    ),'[]'::jsonb) else '[]'::jsonb end,
    'documents',case when pa.can_view_documents then coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',doc.id,'title',doc.title,'original_filename',doc.original_filename,'document_type',doc.document_type,
        'status',doc.status,'mime_type',doc.mime_type,'file_size_bytes',doc.file_size_bytes,
        'storage_bucket',doc.storage_bucket,'storage_path',doc.storage_path,'created_at',doc.created_at
      ) order by doc.created_at desc)
      from public.documents doc where doc.organization_id=pa.organization_id and doc.project_id=pa.id
    ),'[]'::jsonb) else '[]'::jsonb end
  ) as row_json, pa.updated_at
...
select jsonb_build_object(
  ...
  'grants',coalesce((
    select jsonb_agg(jsonb_build_object(
      'id',g.id,'organization_id',g.organization_id,'customer_id',g.customer_id,'project_id',g.project_id,
      'access_level',g.access_level,'can_view_quotes',g.can_view_quotes,'can_view_documents',g.can_view_documents,
      'can_view_deliveries',g.can_view_deliveries,'can_approve',g.can_approve,'expires_at',g.expires_at
    ) order by g.created_at) from active_grants g
  ),'[]'::jsonb),
...
select exists (
  ...
  join public.portal_access_grants g on g.organization_id=d.organization_id
   and g.user_id=auth.uid() and g.status='active' and g.can_view_documents
   and (g.expires_at is null or g.expires_at>now())
  where d.storage_bucket=p_bucket and d.storage_path=p_name
    and (
      (g.project_id is not null and d.project_id=g.project_id)
      or (g.customer_id is not null and (
        d.customer_id=g.customer_id
        or exists (select 1 from public.projects p where p.id=d.project_id and p.organization_id=g.organization_id and p.customer_id=g.customer_id)
      ))
    )
);
$$;
...
create policy "Portal document read access" on storage.objects
